...
describe('CriminalChangePrison.vue 正确加载监狱监区列表，调用获取罪犯信息列表方法 getPrisonInfo()',()=>{
...
describe('CriminalChangePrison.vue 根据转至监狱ID加载监区列表 getToPrisonDepartInfo ()', () => {
  ...
  it('数据层：调用获取监区列表应该传值正确的监狱ID，成功后prisonDepartments正确', () => {
    ...
    expect(promiseCall.calledWithMatch('prisoner/getDepartments', {params: {"prisonId": 2}})).to.equal(true)
    expect(vm.toPrisonDepartments).to.have.lengthOf(1)
...
//     Vue.nextTick(() => {
//       let CriminalChangePrison = vm.$el.querySelector(".criminal-search")
//       expect(vm.isManage).to.equal(false)
//       expect(CriminalChangePrison).to.not.exist
//     })
//     done()
//   })
...
//     Vue.nextTick(() => {
//       let CriminalChangePrison = vm2.$el.querySelector(".criminal-search")
//       expect(vm2.isManage).to.equal(true)
//       expect(CriminalChangePrison).to.be.exist
//     })
//     done()
//   })
...
//     let listFrequency = sinon.spy(vm, "CriminalChangePrison")
//     Vue.nextTick(() => {
...
//     let listFrequency = sinon.spy(vm2, "CriminalChangePrison")
//     Vue.nextTick(() => {
//       expect(listFrequency.callCount).to.equal(1)
//     })
//     done()
...
//     let listFrequency = sinon.spy(vm3, "CriminalChangePrison")
//     Vue.nextTick(() => {
...
// describe('罪犯信息列表获取 CriminalChangePrison()',()=>{
// 	const vm = new Vue(CriminalChangePrison).$mount()
//   let axiosFrequency
