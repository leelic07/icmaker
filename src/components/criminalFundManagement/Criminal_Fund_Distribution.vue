...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      //下载excel
      downLoadTemplate() {
        window.location.href = this.criminal_fund_distribution.downloadExcelUrl;
      },
      //上传excel
      uploadExcel() {
        let self = this;
        $('#table_id_example').on("change", ".file", function (e) {
          ...
          if (self.isExcel(file)) {
            Util.readExcel(file, self, self.criminal_fund_distribution.uploadExcelUrl, store, 'prisonCapitalIncomes', 'dataId', uploadType, prisonName, accountName);
          } else {
            self.remind = {
              status: 'warn',
              msg: '请上传格式正确的Excel文件'
